  ...
  return (
    <form action={formAction} className="space-y-8">
      ...
      <section className="space-y-5">
        <h2 className="font-display text-[19px] font-medium text-ink">Basics</h2>
      ...
      <section className="space-y-6">
        <h2 className="font-display text-[19px] font-medium text-ink">Your lifestyle</h2>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <SegmentOption
                ...
                value="flexible"
                label="Flexible"
                defaultChecked={!lifestyle?.sleep_schedule || lifestyle?.sleep_schedule === 'flexible'}
              />
              <SegmentOption
                name="sleep_schedule"
                value="night_owl"
        ...
        <div className="rounded-[24px] border border-ink/10 bg-paper p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <ChipCheckbox name="smoking" label="Smokes" defaultChecked={!!lifestyle?.smoking} />
            <ChipCheckbox name="has_pets" label="Has pets" defaultChecked={!!lifestyle?.has_pets} />
            <ChipCheckbox
              name="guests_often"
      ...
      {profile.user_type === 'has_place' && (
        <section className="space-y-5">
          <h2 className="font-display text-[19px] font-medium text-ink">Your place</h2>

          <div>
            <label className={labelClass}>Address</label>
            <input name="address" type="text" defaultValue={listing?.address ?? ''} className={inputClass} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>City</label>
              <input name="listing_city" type="text" defaultValue={listing?.city ?? ''} className={inputClass} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="bedrooms" type="number" defaultValue={listing?.bedrooms ?? ''} className={inputClass} />
            </div>
            <div>
      ...
      {state?.error && (
        <p className="text-[13.5px] text-clay border border-clay/20 bg-clay/5 rounded-2xl p-3">{state.error}</p>
      )}

      <div className="flex items-center gap-4 pt-1">
        <SubmitButton />
        {showSaved && <span className="text-[13.5px] text-ink/60">Saved ✓</span>}
      </div>
    </form>
  )
}
